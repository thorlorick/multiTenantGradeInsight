...
class DatabaseManager:
    ...
    async def get_tenant_registry_session(self) -> AsyncGenerator[AsyncSession, None]:
        ...
        if not self.tenant_registry_session_maker:
            raise RuntimeError("Tenant registry session maker not initialized")
        
        # The session_maker is already a context manager, so we can use it directly
